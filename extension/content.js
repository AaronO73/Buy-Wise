(function () {
  ...
  chip.addEventListener('click', async () => {
    ...
    if (!panel.classList.contains('hidden')) {
      panel.innerHTML = '<div>Analyzing current page…</div>';
      try {
        ...
        panel.innerHTML = response.ok
          ? `<strong>${payload.analysis.verdict}</strong><div>Confidence: ${payload.analysis.confidence}</div><div>${payload.analysis.strengthMessage}</div><div class="muted">${payload.analysis.disclaimerMicro}</div><div class="muted">Was this helpful? 👍 / 👎</div>`
